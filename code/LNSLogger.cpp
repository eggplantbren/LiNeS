#include "LNSLogger.h"
#include <fstream>
#include <iomanip>
#include <string>
#include <vector>

namespace LiNeS
{

LNSLogger::LNSLogger()
{

}

LNSLogger::~LNSLogger()
{

}

void LNSLogger::log_level(double logX, const std::vector<double>& scalars)
{
    levels_logX.push_back(logX);
    levels_scalars.push_back(scalars);

    std::fstream fout;
    fout.open("levels_logX.txt", std::ios::out | std::ios::app);
    fout<<std::setprecision(12);
    fout<<levels_logX.size()<<' '<<logX<<std::endl;
    fout.close();
}

void LNSLogger::log_particle_info(unsigned int run_id, size_t level_id,
                                      const std::vector<double>& scalars)
{
    particles_run_id.push_back(run_id);
    particles_level_id.push_back(level_id);
    particles_scalars.push_back(scalars);

    std::fstream fout;

    fout.open("particles_info.txt", std::ios::out | std::ios::app);
    fout<<std::setprecision(12);
    fout<<run_id<<' '<<level_id<<' ';
    for(double s: scalars)
        fout<<s<<' ';
    fout<<std::endl;

    fout.close();
}

void LNSLogger::clear_output_files()
{
    std::vector<std::string> filenames{"levels_scalars.txt",
                                       "levels_logX.txt",
                                       "particles.txt",
                                       "particles_info.txt"};

    for(const auto& filename: filenames)
    {
        std::fstream fout(filename, std::ios::out);
        fout.close();
    }
}

} // namespace LiNeS

